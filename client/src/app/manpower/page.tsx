  ...
  return (
    <div className="w-full">
      ...
      <p className={`text-[11px] mt-1 ${over ? 'text-red-600 font-medium' : 'text-secondary'}`}>
        {formatINR(committed)} / {formatINR(sanctioned)} · {pct.toFixed(0)}%{over ? ' · OVER' : ''}
      </p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select value={status} onChange={setStatus} placeholder="All statuses" options={[...EMPLOYEE_STATUSES]} labels={EMPLOYEE_STATUS_LABELS} />
      </div>
      <div className="bg-card rounded-xl border border-border overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? (
                ...
                <tr key={e.id} className="hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">{e.monthly_ctc ? formatINR(e.monthly_ctc) : '—'}{Number(e.sanction_variance) > 0 && <span className="ml-1 text-[11px] text-red-600">+{formatINR(e.sanction_variance)}</span>}</td>
                  <td className="px-4 py-3"><EmployeeStatusChip status={e.employment_status} />{e.open_to_backfill ? <span className="ml-1 text-[11px] text-orange-600">backfill</span> : null}</td>
            ...
            <tbody className="divide-y divide-border">
              {isLoading ? (
                ...
                <tr key={r.id} className="hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3"><EmployeeStatusChip status={r.employment_status} /></td>
