...
/**
 * The version migration 002 stamped onto everything that existed before it ran: payslips
 * produced by rules this codebase no longer contains. Those can never be reproduced by
 * recomputing, so their months stay closed permanently.
 *
 * Anything at 2 or above was generated by this lineage. A later CALC_VERSION bump changes what
 * a recompute would *produce*, but it does not make those months unreproducible — so it must
 * not retroactively close them.
 */
const LEGACY_CALC_VERSION = 1;

/**
 * Throws if this month must not be written to.
 *
 ...
 * figures the last preview happened to hold. Migration 002 refused to freeze live drafts for
 * exactly this reason ("Freezing a live draft would leave that month impossible to re-run,
 * adjust or correct"), and a version bump must not quietly undo that. An unpaid draft under old
 * rules is precisely the thing that SHOULD be re-run: re-running restamps it at the current
 * version, which is how a month leaves the mixed state rather than being trapped in it.
 */
async function assertMonthNotFrozen(month: number, year: number, consequence: string): Promise<void> {
  const run = await getRun(month, year);
  if (run?.status === 'locked') {
    throw new AppError(`${monthName(month)} ${year} is locked — ${consequence}.`, 409);
  }

  const legacy = await db('payslip_history')
    .where({ month, year }).where('calc_version', '<=', LEGACY_CALC_VERSION).count('id as c').first();
  if (Number(legacy?.c ?? 0) > 0) {
    throw new AppError(
      `${monthName(month)} ${year} was calculated under rules that no longer exist and is frozen — ${consequence}.`,
      409,
    );
  }
}
